...
async function renderPartidos() {
    ...
    if(snapP.exists()){
        ...
        const obtenerBandera = (equipo) => {
            ...
            if (codigo) {
                return `<img src="https://flagcdn.com/w80/${codigo}.png" class="flag-img" alt="${equipo}">`;
            }
...
window.verApuestasGlobales = async (idPartido, equipoL, equipoV) => {
    ...
    Swal.fire({
        title: `<span style="font-family:Anton; color:var(--accent-gold); font-size:1.4rem;">PRONÓSTICOS REGISTRADOS</span><br><span style="font-size:0.9rem; color:#fff;">${equipoL} vs ${equipoV}</span>`,
        background: '#020d1a', color: '#fff', width: '450px', html: tablaHtml, confirmButtonText: 'CERRAR', confirmButtonColor: '#333'
    });
};
...
window.guardarTodosLosPronosticos = async () => {
    try {
        ...
        if (hayCambios) {
            await update(ref(db, `pronosticos/${currentCedula}`), up);
        }
        
        Swal.fire("Éxito", "Tus pronósticos han sido actualizados", "success").then(() => {
            renderPartidos();
        });
...
async function calcularMisPuntos() {
    ...
    if (currentCedula && usuarios[currentCedula]) {
        document.getElementById('mis-puntos-total').innerText = updatesUsuarios[`usuarios/${currentCedula}/puntos`] || 0;
    }
}
        ...
        sorted.forEach((u, i) => {
            ...
            htmlRanking += `
                <div style="display:flex; justify-content:space-between; padding:10px; border-bottom:1px solid rgba(255,255,255,0.05)">
                    <span style="font-weight: 600;">${esMismoPuntajeAnterior ? '&nbsp;&nbsp;&nbsp;&nbsp;' : puestoActual + '.'} ${u.nombre}</span>
                    <span style="color:var(--accent-gold); font-weight:bold">${u.puntos} PTS</span>
                </div>`;
        });
        ...
        Object.entries(snapP.val()).forEach(([id, p]) => {
            ...
            list.innerHTML += `
                <div style="margin-bottom:10px; padding:10px; border-bottom:1px solid #333">
                    ${p.equipoL} vs ${p.equipoV} (${p.grupo})
                    <div style="display:flex; gap:5px; margin-top:5px">
                        <input type="number" id="adm_l_${id}" class="input-score" style="width:40px; height:40px" value="${resOf[id]?.golesL ?? ''}" ${tieneResultado ? 'disabled' : ''}>
                        <input type="number" id="adm_v_${id}" class="input-score" style="width:40px; height:40px" value="${resOf[id]?.golesV ?? ''}" ${tieneResultado ? 'disabled' : ''}>
                    </div>
                </div>`;
        });
    }
...
window.cargarPartidosMasivos = async () => {
    ...
    try {
        ...
        for (let i = 0; i < partidos.length; i++) {
            const p = partidos[i];
            const eqL = p.local || p.equipoL;
            const eqV = p.visitante || p.equipoV;
            const grp = p.grupo;
            const fecha = p.fecha_hora || p.fechaInicio;

            if(eqL && eqV && grp && fecha) {
                const uniqueId = 'p_' + (Date.now() + i);
                await set(ref(db, `partidos_oficiales/${uniqueId}`), { 
                    equipoL: eqL, 
                    equipoV: eqV, 
                    grupo: grp, 
                    fechaInicio: fecha 
        ...
        renderAdminList();
        Swal.fire("Sincronización Exitosa", `Se importaron ${contadorAgregados} partidos correctamente a la base de datos.`, "success");
        
    } catch (err) {
        Swal.fire("Error de Formato JSON", "Asegúrate de copiar el JSON exacto. Detalle: " + err.message, "error");
    }
};
